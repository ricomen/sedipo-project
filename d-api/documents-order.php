...
while($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
    //$user_age = $date_order - $row['date_of_birth'];
    $customers[] =  ['lastname'=>$row['lastname'], 'firstname'=>$row['firstname'], 'middlename'=>$row['middlename'], 'position_name'=>$row['position_name'], 'date_of_birth'=>$row['date_of_birth'], 'user_age'=>$user_age, 'snils'=>$row['snils'], 'address'=>$row['address'], 'pasport_series'=>$row['pasport_series'], 'pasport_number'=>$row['pasport_number'], 'pasport_unit'=>$row['pasport_unit'], 'pasport_unit_number'=>$row['pasport_unit_number'], 'email'=>$row['email'], 'phone'=>$row['phone'], 'contr_id'=>$contr_id, 'fullname3'=>formatFio($row['lastname'].' '.$row['firstname'].' '.$row['middlename'])  ];
...
//print_r($customer);
//print_r(intval(count($courses)));
//print_r($users[0]);

$customerdate_of_birth = date('d.m.Y',strtotime($customer['date_of_birth']));

if($output_format=="word") {


$phpWord->setValue('customer_lastname', $customer['lastname']);   
$phpWord->setValue('customer_firstname', $customer['firstname']);
$phpWord->setValue('customer_middlename', $customer['middlename']);
$phpWord->setValue('customer_position_name', $customer['position_name']);
$phpWord->setValue('customer_date_of_birth', $customerdate_of_birth);
$phpWord->setValue('customer_snils', $customer['snils']);
$phpWord->setValue('customer_address', $customer['address']);
$phpWord->setValue('customer_phone', $customer['phone']);
...
$payer_id = 0;
$counterparty_id = intval($_GET['counterparty_id']);
if( $order_id >0 ) {
    ...
    foreach($a_contract_id as $v_contract) {
        ...
        if($row2 = $stmt2->fetch(PDO::FETCH_ASSOC)) {
                if( intval($row2['longtime_contract']) >0 && $row['longtime_contract'] == 'true') 
                     ...
                     $contract_order_id = intval($_GET['id']);

                $stmt3 = $dbh->prepare("SELECT `legacy`, DATE_FORMAT(`date_contract`, '%d.%m.%Y') as `date_contract`  FROM `a_counterparty_contract`  WHERE `contract_id`=?  AND `addition`=-1  AND  `counterparty_id`=? AND `order_id`=? ");
                $stmt3->execute( [ intval($v_contract), $counterparty_id, $contract_order_id ] );
                if($row3 = $stmt3->fetch(PDO::FETCH_ASSOC)) { 
                    if($row3['legacy'] != '') {
                        //if($legacy_count == 0)
...
$data3 = [];
if( intval($_GET['contract_id']) >0 ) {
//file_put_contents("lst.txt", print_r($_GET['counterparty_id'], true) );
    ...
    if($row0 = $stmt0->fetch(PDO::FETCH_ASSOC)) {
        if( intval($row0['longtime_contract']) >0 && $longtime_contract_c == true  && $counterparty_id>1)
                $contract_order_id = 0;
        else
                $contract_order_id = intval($_GET['id']);

        if( $counterparty_id) {
            $stmt3 = $dbh->prepare("SELECT `legacy`, `date_contract`, `order_id`  FROM `a_counterparty_contract`  WHERE `contract_id`=? AND  `counterparty_id`=? AND `addition`= -1 AND `order_id`=? ");
            $stmt3->execute( [ intval($_GET['contract_id']), $counterparty_id, $contract_order_id  ] );
            if($row3 = $stmt3->fetch(PDO::FETCH_ASSOC)) {
                 if($row3['legacy'] != ''){
...
if( $payer_id>1 ) {
    ...
    if($output_format=="word") {
        ...
        if(isset($_GET['print_v']) && $_GET['print_v']=='false' && $row['enterprise_manager_signs']!='') {
            $phpWord->setImageValue('payer-enterprise_manager_signs', '../documents/'.$row['enterprise_manager_signs']);
        }
        else {
            $phpWord->setValue('payer-enterprise_manager_signs', '______________________');
        }
...
if($output_format=="word") {
      ...
      if(isset($_GET['print_v']) && $_GET['print_v']=='false' && $row2['enterprise_manager_signs']!='') {
          $phpWord->setImageValue('self-enterprise_manager_signs', '../documents/'.$row2['enterprise_manager_signs']);
      }
      else {
          $phpWord->setValue('self-enterprise_manager_signs', '______________________');
      }  
...
if($performer_id == 0){
    if($output_format=="word") {
        ...
        if(isset($_GET['print_v']) && $_GET['print_v']=='false' && $row2['enterprise_manager_signs']!='') {
            $phpWord->setImageValue('performer-enterprise_manager_signs', '../documents/'.$row2['enterprise_manager_signs']);
        }
        else {
            $phpWord->setValue('performer-enterprise_manager_signs', '______________________');
        }
...
while($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
    ...
    if($output_format=="word" && $is_students) {
          ...
          $phpWord->setValue('students-self-name'.'#'.$row_number, $self_data['name']);
          $phpWord->setValue('students-self-shortname'.'#'.$row_number, $self_data['shortname']);
          $phpWord->setValue('students-self-inn'.'#'.$row_number, $self_data['inn']);
          $phpWord->setValue('students-self-kpp'.'#'.$row_number, $self_data['kpp']);
          $phpWord->setValue('students-self-ogrn'.'#'.$row_number, $self_data['ogrn']);
          $phpWord->setValue('students-self-addres1'.'#'.$row_number, $self_data['addres1']);
          $phpWord->setValue('students-self-addres2'.'#'.$row_number, $self_data['addres2']);
          $phpWord->setValue('students-self-city'.'#'.$row_number, $self_data['city']);
          $phpWord->setValue('students-self-email'.'#'.$row_number, $self_data['email']);
          $phpWord->setValue('students-self-phone'.'#'.$row_number, $self_data['phone']);
          $phpWord->setValue('students-self-position_head'.'#'.$row_number, $self_data['position_head']);
          $phpWord->setValue('students-self-enterprise_manager'.'#'.$row_number, $self_data['enterprise_manager']);
          $phpWord->setValue('students-self-position_head2'.'#'.$row_number, $self_data['position_head2']);
          $phpWord->setValue('students-self-enterprise_manager2'.'#'.$row_number, $self_data['enterprise_manager2']);
          $phpWord->setValue('students-self-enterprise_manager3'.'#'.$row_number, $self_datarprise_manager3);
          //$phpWord->setValue('students-self-enterprise_manager_signs'.'#'.$row_number, $self_data['enterprise_manager_signs']);
          $phpWord->setValue('students-self-bank'.'#'.$row_number, $self_data['bank']);
          $phpWord->setValue('students-self-checking_account'.'#'.$row_number, $self_data['checking_account']);
          $phpWord->setValue('students-self-correspondent_account'.'#'.$row_number, $self_data['correspondent_account']);
          $phpWord->setValue('students-self-bik'.'#'.$row_number, $self_data['bik']);
          $phpWord->setValue('students-self-license'.'#'.$row_number, $self_data['license']);
          $phpWord->setValue('students-self-accreditation'.'#'.$row_number, $self_data['accreditation']);
          ...
          if(isset($_GET['print_v']) && $_GET['print_v']=='false') {
              $phpWord->setImageValue('students-self-stamp', $imageStampStyle);
        }

          if(isset($_GET['print_v']) && $_GET['print_v']=='false' && $self_data['enterprise_manager_signs']!='') {
            $phpWord->setImageValue('students-self-enterprise_manager_signs'.'#'.$row_number, '../documents/'.$self_data['enterprise_manager_signs']);
          }
          else {
              $phpWord->setValue('students-self-enterprise_manager_signs'.'#'.$row_number, '______________________');
          }
  

          
          $phpWord->setValue('students-performer-name'.'#'.$row_number, $center_data['name']);
          $phpWord->setValue('students-performer-shortname'.'#'.$row_number, $center_data['shortname']);
          $phpWord->setValue('students-performer-inn'.'#'.$row_number, $center_data['inn']);
          $phpWord->setValue('students-performer-kpp'.'#'.$row_number, $center_data['kpp']);
          $phpWord->setValue('students-performer-ogrn'.'#'.$row_number, $center_data['ogrn']);
          $phpWord->setValue('students-performer-addres1'.'#'.$row_number, $center_data['addres1']);
          $phpWord->setValue('students-performer-addres2'.'#'.$row_number, $center_data['addres2']);
          $phpWord->setValue('students-performer-city'.'#'.$row_number, $center_data['city']);
          $phpWord->setValue('students-performer-email'.'#'.$row_number, $center_data['email']);
          $phpWord->setValue('students-performer-phone'.'#'.$row_number, $center_data['phone']);
          $phpWord->setValue('students-performer-position_head'.'#'.$row_number, $center_data['position_head']);
          $phpWord->setValue('students-performer-enterprise_manager'.'#'.$row_number, $center_data['enterprise_manager']);
          $phpWord->setValue('students-performer-position_head2'.'#'.$row_number, $center_data['position_head2']);
          $phpWord->setValue('students-performer-enterprise_manager2'.'#'.$row_number, $center_data['enterprise_manager2']);
          $phpWord->setValue('students-performer-enterprise_manager3'.'#'.$row_number, $center_datarprise_manager3);
          //$phpWord->setValue('students-performer-enterprise_manager_signs'.'#'.$row_number, $center_data['enterprise_manager_signs']);
          $phpWord->setValue('students-performer-bank'.'#'.$row_number, $center_data['bank']);
          $phpWord->setValue('students-performer-checking_account'.'#'.$row_number, $center_data['checking_account']);
          $phpWord->setValue('students-performer-correspondent_account', $row2['correspondent_account']);
          $phpWord->setValue('students-performer-bik'.'#'.$row_number, $center_data['bik']);
          $phpWord->setValue('students-performer-license'.'#'.$row_number, $center_data['license']);


          if(isset($_GET['print_v']) && $_GET['print_v']=='false' && $center_data['enterprise_manager_signs']!='') {
            $phpWord->setImageValue('students-performer-enterprise_manager_signs'.'#'.$row_number, '../documents/'.$center_data['enterprise_manager_signs']);
          }
          else {
              $phpWord->setValue('students-performer-enterprise_manager_signs'.'#'.$row_number, '______________________');
          }

          $phpWord->setValue('students-contract_number'.'#'.$row_number, $contract_number_nodate);
          $phpWord->setValue('students-date_contract'.'#'.$row_number, $date_contractf);
