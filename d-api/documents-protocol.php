...
//print_r($protocol_template);
    ...
    $course =  [ 'name'=>$course_name,  'hours'=>$row['hours'],  'date_begin'=>$row['date_begin'], 'date_end'=>$row['date_end'], 'date_protocol'=>$row['date_protocol'], 'date_protocol_rus'=>formatRussianDate($row['date_protocol']), 'protocol_num'=>$protocol_number, 'category_name'=>$category_name, 'competence'=>$row['competence'], 'qualification'=>$row['qualification'], 'area'=>$row['area'], 'list_documents_num'=>$list_documents_num ];
...
//print_r($data);
 ...
 while($row2 = $stmt2->fetch(PDO::FETCH_ASSOC)) {
    ...
    if($protocol_template['is_dateprotocol']=='true')
        $user_row[] =  date("d.m.Y", strtotime($date_protocol)).'<br /> № '.$course['protocol_num'];
...
if($groupid > 0){
    ...
    out_protocol($groupid, $order_id,  $groupid );
}
...
*/
      $html_str =  report_protocol($row0->cohort_id, $order_id ) ;
      $out_flses[] = out_protocol($row0->cohort_id, $order_id, $groupid  );
      $j++;
    }
    $report_file = 'Protocol_order'.$order_id;
//print_r($out_flses);
    merge_protocol($out_flses  );
}
...
function out_protocol($groupid, $order_id, $mode  ){
 global  $dbh, $smarty, $JsonApiURL, $is_short_certificate_num, $IS_SECR;
 global   $report_file, $print_v;
 global $output_format, $page_size;
 global $phpWord;

 global $html_str, $footer_html_str, $sessionid;
 ...
 else if( $output_format=="word" && $print_switch!='false' )
 {
  ...
  $location = '../tmp/' . $sessionid. '_'.$cohort_id .'-protocol';
	...
	$dompdf->render();
        if($mode>0){
             $dompdf->stream(rus2translit($report_file) . ".pdf", array("Attachment" => 0));
               ...
               $rc = '../tmp/'.$sessionid.'_'.$order_id.'_'.$groupid. '.pdf';
...
function merge_protocol($out_flses  ){
 global  $dbh, $smarty, $JsonApiURL, $is_short_certificate_num, $IS_SECR;
 global   $report_file, $print_v;
 global $output_format, $page_size;
 global $phpWord;
