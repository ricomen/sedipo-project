...
while($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
    ...
    if($output_format=="word" && $is_students) {
          ...
          $phpWord->setValue('students-date_of_birth'.'#'.$row_number, $row['date_of_birth']);
...
while($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
    ...
    $users2 = [];
    $cohort_id = 0;
    $stmt2 = $dbh->prepare("SELECT  DISTINCT  `lastname`, `firstname`, `middlename`, `date_of_birth`, `snils`, `address`, `level_of_education`, `pasport_series`, `pasport_number`, `pasport_unit`, `pasport_unit_number`, `phone`, `a_job_title`.`name` as `position_name`, `a_user_counterparty`.`user_id`, `number_of_people`    FROM `a_order_users`  LEFT JOIN `a_user_counterparty` USING(`user_counterparty_id`)   LEFT JOIN `a_users` ON `a_user_counterparty`.`user_id`=`a_users`.`user_id`    LEFT JOIN `a_job_title` USING(`job_title_id`)  LEFT JOIN `a_order_course` USING(`item_id`)    WHERE `a_user_counterparty`.`user_id`>0  AND `order_id`=? AND `course_id`=? AND `a_users`.status=0 ".$lock_str." ORDER BY `lastname`,`firstname` ");
    $stmt2->execute( [$order_id , $row['course_id'] ] );
    while($row2 = $stmt2->fetch(PDO::FETCH_ASSOC)) {  
        $users2[] = ['lastname'=>$row2['lastname'], 'firstname'=>$row2['firstname'], 'middlename'=>$row2['middlename'], 'level_of_education'=>$education_levels[$row2['level_of_education']], 'job_title'=>$row2['position_name'], 'position_name'=>$row2['position_name'], 'date_of_birth'=>$row2['date_of_birth'], 'snils'=>$row2['snils'], 'address'=>$row2['address'], 'pasport_series'=>$row2['pasport_series'], 'pasport_number'=>$row2['pasport_number'], 'pasport_unit'=>$row2['pasport_unit'], 'pasport_unit_number'=>$row2['pasport_unit_number'],  'phone'=>$row2['phone'], 'fullname3'=>formatFio($row2['lastname'].' '.$row2['firstname'].' '.$row2['middlename'])  ];
    ...
    if($output_format=="word" && $is_courses) {
          ...
          $phpWord->setValue('courses-date_begin'.'#'.$row_number, $date_begin);
          $phpWord->setValue('courses-date_end'.'#'.$row_number, $date_end);
          $phpWord->setValue('courses-date_protocol'.'#'.$row_number, $date_protocol);
...
if($output_format=="word") {
      ...
      $phpWord->setValue('date_order', $order_date_order);
      $phpWord->setValue('number', $row['number']);
      $phpWord->setValue('date_begin', $order_date_begin);
      $phpWord->setValue('date_end', $order_date_end);
      $phpWord->setValue('date_completed',  $date_completed);
      ...
      $phpWord->setValue('date_order_finish', $date_order_finish);
      ...
      $phpWord->setValue('contract_number', $contract_number_s);
      $phpWord->setValue('date_contract', $date_contract);
      $phpWord->setValue('date_contract_finish', $date_contract_finish);
}
else {
