...
//    updated() {
    ...
    if(this.counterparty_id>0){
      ...
      axios
      .post(JsonApiURL+'api/account_json.php', {object: {objectId: this.counterparty_id, sessionId: session_t.sessionId } }, {withCredentials: true})
      .then(response => { 
      ...
      axios
      .post(JsonApiURL+'api/auth_json.php', {is_auth: { sessionId: session_t.sessionId } }, {withCredentials: true})
      .then(response => { 
            this.info1 = response.data
     ...
     axios
      .post(JsonApiURL+'api/helpers_json.php', {table: 'a_counterparty_edit'}, {withCredentials: true})
      .then(response => {
            ...
            if(Object.keys(data).length) {
                ...
                labels.forEach(label => {
                    ...
                    if(getFor in this.helpers) {
                        const help_block = ` <a title='${this.helpers[getFor]}'><i class="fa-regular fa-circle-question text-primary"></i></a>`;
                        label.innerHTML = label.innerHTML + (help_block);
                    }
                    // добавляем нормативку
                    if(getFor in this.normative) {
                        const help_block = ` <a title='${this.normative[getFor]}'><i class="fa-solid fa-book text-primary"></i></a>`;
                        label.innerHTML = label.innerHTML + (help_block);
                    }
  ...
  methods: {
            ...
            .then(response => {
    	      ...
    	      //this.info9 = response.data
               ...
               if(fl) {
		        ...
		        axios
        	    .post(JsonApiURL+'api/account_json.php', {update: {objectId: this.account_id, login: this.email1,  fullname: this.name,  email: this.email_contact,  phone: this.phone_contact,   sessionId: session_t.sessionId } }, {withCredentials: true})
        	    .then(response2 => {
	...
	if(fl){      
	    axios
            .post(JsonApiURL+'api/account_json.php', {insert: { login: this.email1,  fullname: this.name,   email: this.email_contact,  phone: this.phone_contact,   role: 'counterparty',  sessionId: session_t.sessionId } }, {withCredentials: true})
            .then(response => {
              console.log(response)
              ...
              this.account_id = response.data.result
				...
				axios
        			...
        			.then(response3 => {
            			    ...
            			    if(response3.data.status==0) {
                                        this.counterpartyCreate( objectId)
                                    }
        			})
        			.catch(error3 => {
            			    console.log(error3.response)
      ...
      check_unique() {
        ...
        var status = 0
        axios
          .post(JsonApiURL+'api/counterparty_check_json.php', {check_unique: {is_create: this.counterparty_id, objectId: this.counterparty_id, account_id: this.counterparty_id, inn: this.inn, name: this.name, shortname: this.shortname,  email: this.email1, sessionId: session_t.sessionId } }, {withCredentials: true})
          .then(response => { 
              console.log(response)
                this.info9 = response.data
      ...
      check_required(fl) {
        ...
        if(this.email1 == '' || this.email1 == ' ') {
            if(fl>0){
                status = status +1
                alert("При незапллненном поле 'Официальный email', Создать личный кабинет заказчика, невозможно")
            }
            else if(this.email_contact == '' || this.email_contact == ' ' )
                alert("При незапллненном поле 'Официальный email', функционал системы будет ограничен")
        }
...
/*        axios
    ...
    load_combo_egrul(){
      ...
      if(this.counterparty_с!=''){
       axios
        //.post(JsonApiURL+'api/egrul_json.php', {list: {conditions: {"type": 0}, search: this.name, sessionId: session_t.sessionId }})
        .post(JsonApiURL+'api/egrul_json.php', {list: { search: this.inn, sessionId: session_t.sessionId }}, {withCredentials: true})
        .then(response => { 
              this.info3 = response.data
              this.wait = 0
              console.log(response)
    ...
    load_combo_names(){
      ...
      if(this.name!=''){
       axios
        //.post(JsonApiURL+'api/egrul_json.php', {list: {conditions: {"type": 0}, search: this.name, sessionId: session_t.sessionId }})
        .post(JsonApiURL+'api/egrul_json.php', {list: { search: this.name, sessionId: session_t.sessionId }}, {withCredentials: true})
        .then(response => { 
    ...
    search_combo2(index){
       this.inn = this.info2.list[index].inn
       this.name = this.info2.list[index].name 
       this.shortname  = this.info2.list[index].name.substring(0, 250)
       this.info2.list = []
       this.load_egrul()
    },

    search_combo3(index){
       this.inn = this.info3.list[index].inn
       this.name = this.info3.list[index].name 
       this.shortname  = this.info3.list[index].name.substring(0, 250)
       this.info3.list = []
       this.load_egrul()
    ...
    load_egrul(){
      axios
      .post(JsonApiURL+'api/egrul_json.php', {object: {objectId: this.inn, sessionId: session_t.sessionId } }, {withCredentials: true})
      .then(response => { 
            this.info9 = response.data
            this.email1 = this.info9.result.email
            this.phone1 = this.info9.result.phone
            this.inn = this.info9.result.inn
            this.kpp = this.info9.result.kpp
            this.ogrn = this.info9.result.ogrn
            this.enterprise_manager = this.info9.result.enterprise_manager
            this.addres1 = this.info9.result.addres1
            this.bik = this.info9.result.bik
        })
	...
	template: `
  <container v-if="role_privileges.counterparty_list == 2">
  <div><navigation></navigation><h3>Редактирование реквизитов контрагента</h3> 
...
<div class="container">
  ...
  <br /> 
  <div v-if="role_privileges.counterparty_list == 2 && counterparty_id!=1"   class="mb-2 row">
  ...
  <br />
   ...
   <label for="input_inn" class="form-label col-sm-2">ИНН</label>
    <div class ="container col">
     <!--<input v-model="inn"   class="form-control" id="input_inn"   type="text"  @change="check_unique()"  @input="load_combo_egrul()" >-->
     <input v-model="inn"   class="form-control" id="input_inn"   type="text"  @change="check_unique()"  ><span v-if="wait"><i class="fa-solid fa-cog fa-spin"></i></span>
	<ul class ="list-group" id ="listItemINN" style="text-align: left;">
		<li  v-for="(item3, index) in info3.list" class ="list-group" @click="search_combo3(index)"  style="cursor: pointer;" > {{item3.inn}} - {{item3.name.substring(0, 72)}}</li>
	</ul>
    </div>
     <div class="col-1" > <button @click="load_combo_egrul()" type="button" class="btn btn-light" title="Поиск"><i class="fa-solid fa-magnifying-glass"></i></button></div>
  ...
  <div class="mb-2 row">
    ...
    <div class ="container col">
      <input v-model="name"   class="form-control" id="input_name"   type="text" @change="check_unique()"  @input="load_combo_names()"  >
	<ul class ="list-group" id ="listItemName" style="text-align: left;">
		<li  v-for="(item2, index) in info2.list" class ="list-group" @click="search_combo2(index)"  style="cursor: pointer;" > {{item2.name.substring(0, 72)}}</li>
	</ul>
    </div>
    <div class="col-1" style="color: red"> <a title="Обязательное для заполнения поле"><b>*</b></a> </div>
  </div>
...
<!--  <div class="mb-2">	
  ...
  <div class="mb-2 row">	
    ...
    <div class="col-4" style="color: green"> <a title="Поле, заполнение которого может быть обязательным"><b>*</b></a> </div>
  </div>
 ...
 <span v-if="entity != 0">
  ...
  <label for="input_enterprise_manager" class="form-label col-sm-3" >Руководитель</label>
    <input v-model="enterprise_manager"   class="form-control col" id="input_enterprise_manager"   type="text" >
     <div class="col" ><input v-model="enterprise_manager2" class="form-control col" id="input_enterprise_manager2"   type="text" readonly onfocus="this.removeAttribute('readonly')"></div>
  </div>
 </span>
 ...
 <!--<sapan v-if="counterparty_id!=1">  
  <hr />
  <div class="mb-2 row">
  <label for="input_contact" class="form-label col-sm-3"></label>
     <div class="col"><b>Контактное лицо</b></div>
  </div>
 ...
 <sapan v-if="counterparty_id!=1">  
  ...
  <div class="mb-2">	
    <div align="right">
    <button v-if="is_LMS==1 && counterparty_id>1 && role_privileges.counterparty_list == 2 && c_type==0 && is_DEVELOPMENT==1"  class="btn btn-outline-success"    @click="accountCheckSave(1)"> Создать личный кабинет заказчика </button>
    &nbsp<button v-if="counterparty_id>0"  class="btn btn-primary"    @click="accountCheckSave(0)"> Сохранить </button>

    <button v-if="is_LMS==1 && counterparty_id==0 && role_privileges.counterparty_list == 2 && c_type==0  && is_make==0 && is_DEVELOPMENT==1"  class="btn btn-outline-success"    @click="accountCheckCreate(1)"> Создать учетную запись и личный кабинет заказчика </button>
    &nbsp<button v-if="counterparty_id==0 && role_privileges.counterparty_list == 2 && is_make>0 "  class="btn btn-primary"    @click="accountCheckCreate(0)"> Создать учетную запись <i class="fa-solid fa-circle-right"></i> </button>
    &nbsp<button v-if="counterparty_id==0 && role_privileges.counterparty_list == 2 && is_make<=0"  class="btn btn-primary"    @click="accountCheckCreate(0)"> Создать учетную запись </button>
