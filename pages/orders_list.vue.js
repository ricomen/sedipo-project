var OrdersList = {
  data: function () {
   ...
   mounted() {
    ...
    axios
      .post(JsonApiURL+'api/orders_json.php', {list: {date1: this.datesearch, date2: this.datesearch2, order_name: this.ordersearch, course: this.namesearch, status: status,  counterparty_id: counterparty_id,  orderid: this.a_order_id, page: this.page, sort: this.a_sort,   sessionId: session_t.sessionId }}, {withCredentials: true})
      .then(response => { 
  ...
  methods: {
    ...
    var status = this.status
     ...
     axios
      .post(JsonApiURL+'api/orders_json.php', {list: {date1: this.datesearch, date2: this.datesearch2, order_name: this.ordersearch, course: this.namesearch, status: status, counterparty_id: this.a_counterparty_id, orderid: this.a_order_id, page: this.page, sort: this.a_sort,  sessionId: session_t.sessionId }}, {withCredentials: true})
      .then(response => { 
            this.info = response.data
            this.role = this.info.role
            this.numPages = this.info.numPages
      ...
      newStatus(order_id, a_status, is_modal) {
        ...
        if(is_modal == 2 && this.payment_receipt>0 && this.payment_receipt_date!=null && this.payment_receipt_date!='' && this.payment_receipt_date!='0000-00-00'){
                axios
                .post(JsonApiURL+'api/orders_json.php', {payment_receipt: {objectId: orderId, payment_receipt: this.payment_receipt, payment_receipt_date: this.payment_receipt_date,  sessionId: session_t.sessionId }}, {withCredentials: true})
                .then(response => { 
        ...
        if(fl) {
            axios
            .post(JsonApiURL+'api/orders_json.php', {update_status: { objectId: orderId,  status: status,  sessionId: session_t.sessionId }}, {withCredentials: true})
            .then(response => { 
                //this.info9 = response.data
                  console.log(response)
                axios
                .post(JsonApiURL+'api/orders_json.php', {list: { counterparty_id: this.a_counterparty_id, orderid: this.a_order_id, page: this.page,  sessionId: session_t.sessionId }}, {withCredentials: true})
                .then(response2 => { 
      ...
      orderSync(order_id){
        axios
          .post(JsonApiURL+'api/orders_json.php', {sync: {objectId: order_id,  sessionId: session_t.sessionId }}, {withCredentials: true})
           .then(response => { 
        })
        .catch(error => {
              console.log(error.response)
    ...
    tab_go(month) {
     if(this.year_tab>0) {    
        ...
        axios
        .post(JsonApiURL+'api/orders_json.php', {list: {date1: date_t1, date2: date_t2,  counterparty_id: this.a_counterparty_id, page: this.page,  sessionId: session_t.sessionId }}, {withCredentials: true})
        .then(response => { 
            this.info = response.data
            this.numPages = this.info.numPages
	        this.page = this.info.page
          ...
          .then(response => { 
            //this.info9 = response.data
                console.log(response)
                         ...
                         axios
                          .post(JsonApiURL+'api/orders_json.php', {list: {date1: this.datesearch, date2: this.datesearch2, course: this.namesearch, status: status,  counterparty_id: counterparty_id,  orderid: this.a_order_id, page: this.page, sort: this.a_sort,   sessionId: session_t.sessionId }}, {withCredentials: true})
                          .then(response2 => { 
                              this.info = response2.data
                          })
                          .catch(error => {
...
//this.contract_modal_contract_id = contract_id
        ...
        for (var i = 0; i < contract_list.length; i++) {
            ...
            this.legacy_contract[i] = contract_list[i].legacy_contract
            this.date_contract[i] = contract_list[i].date_contract
        }
...
/*     contractSave() {
        if(this.modal_counterparty_id > 0 ){
            
            axios
              //.post(JsonApiURL+'api/counterparty_contract_json.php', {update: {counterparty_id: this.modal_counterparty_id, date_contract: this.date_contract, a_contract_id: this.a_contract_id, enterprise_manager: this.enterprise_manager, enterprise_manager2: this.enterprise_manager2, enterprise_manager_signs: this.enterprise_manager_signs, sessionId: session_t.sessionId } })
              .post(JsonApiURL+'api/orders_json.php', {contract_update: {counterparty_id: this.modal_counterparty_id, order_id: this.modal_order_id, a_contract_id: this.a_contract_id,  sessionId: session_t.sessionId } })
              .then(response => { 
                 this.contractUpload()            
                 console.log(response)
            })
...
//this.contractUpload()
        ...
        if(this.modal_counterparty_id > 0 && this.modal_order_id>0 ){
            
            axios
              .post(JsonApiURL+'api/counterparty_contract_json.php', {update_order_contract: {order_id: this.modal_order_id, counterparty_id: this.modal_counterparty_id,  legacy_contract: this.legacy_contract,  date_contract: this.date_contract, a_contract_id: this.a_contract_id,  sessionId: session_t.sessionId } }, {withCredentials: true})
              .then(response => { 
                 console.log(response)
                 this.contractUpload()
            })
     ...
     contractUpload() {
        ...
        for (var i = 0; i < this.importfile1.length; i++) {  
            ...
            if(this.importfile1_file_type[i] == 'application/pdf'){
                ...
                formData.append('counterparty_id', this.modal_counterparty_id); 
                if(i == 0 )
                    formData.append('contract_id', 0 ); 
                else
                    formData.append('contract_id', this.a_contract_id[ parseInt((i-1)/5) ]); 
                formData.append('order_id', this.modal_order_id); 
                //formData.append('contract_id', this.modal_contract_lis[(i-1)/4].contract_id); 	
                if( i==0){
                    formData.append('date_contract', this.modal_date_completed); 
                }
                else if( (i-1)%5 == 0 ){
                    if( this.date_contract[(i-1)/5] == null){
                        alert("Не введена дата договора")
                        this.date_exist = false
                        return 
                    }
                    formData.append('date_contract', this.date_contract[(i-1)/5]);
                    formData.append('legacy_contract', this.legacy_contract[(i-1)/5]);
                }
                else
                    formData.append('date_contract', this.modal_date_order); 

                formData.append('addition', (i-1)%5 - 1);
                formData.append('sessionId', session_t.sessionId); 

                axios
                   .post(JsonApiURL+'api/order_contract_upload_json.php', formData, {headers: {'Content-Type': 'multipart/form-data'}}, {withCredentials: true})
                   .then(response => {
                    this.info9 = response.data
                    if( i ==  this.importfile1.length ){
                         this.wait = 0;
     ...
     handleUploadC(index, upload_file) {
            this.importfile1[index*5 +1 ] = document.getElementById('input_contract_file_'+index+'_').files[0];
            this.importfile1_filename[index*5  +1  ] = this.importfile1[index*5 +1 ].name;
            this.importfile1_file_type[index*5 +1  ] = this.importfile1[index*5 +1 ].type;
            if(upload_file=='' && (this.date_contract[index]=='' || this.date_contract[index]==null ||  this.date_contract[index]=='1000-01-01' ) && index>0 )
                 this.date_exist = false
            //this.checkDate(index)
     },
     handleUpload(index, n) {
            this.importfile1[index*5 + n  +2  ] = document.getElementById('input_contract_file_'+index+'_'+n).files[0];
            this.importfile1_filename[index*5 + n  +2  ] = this.importfile1[index*5 + n +2 ].name;
            this.importfile1_file_type[index*5 + n  +2  ] = this.importfile1[index*5 + n +2 ].type;
            //this.checkDate(index)
     },
     ...
     checkDate(index) {
            ...
            if( (this.importfile1[index*5  +1 ]!=null || this.legacy_contract[index]!='') && (this.date_contract[index]=='' || this.date_contract[index]==null ||  this.date_contract[index]=='1000-01-01' ) && index>0  && (index-1)%5==0 )
                  this.date_exist = false
     },
           ...
           axios
              ...
              .then(response => { 
	            ...
	            this.page = this.info.page
                    ...
                    this.$router.push({ name: 'orders_list', params: {counterpartyid: this.a_counterparty_id, orderid: this.a_order_id, sort: this.a_sort }}, {withCredentials: true}) 
       })
      .catch(error => {
              console.log(error.response)
        })
	...
	template: `<div><navigation></navigation><h3>Заявки на обучение </h3> 
           ...
           <td align="left" style="padding-left: 1px; padding-right: 1px; padding-top: 10px;">
            ...
            <td align="center"  style="padding-right: 15px;">
              <p><i>{{item.status_name}}</i></p>
               <span v-if="item.status_id>=5  && item.status_id<=12 && item.groups_count==0">
                    <p><b><small>Необходимо сформировать учебные группы <i class="fa-regular fa-circle-right"></i></small></b><p>
               </span>
               <span v-if="item.status_id>=5  && item.status_id<=12 && item.groups_count>0 && item.lstream_count<item.groups_count ">
                    <p><b><small>Необходимо сформировать расписание учебных групп <i class="fa-regular fa-circle-right"></i></small></b><p>
               </span>
              <div v-if="item.status_id<5  || item.status_id>=10 || (item.groups_count>0 && item.lstream_count==item.groups_count)"   class="d-grid gap-1">
            ...
            <td align="center"> 
                    ...
                    <span  v-if="item.status_id>5 && (item.groups_count==0 || item.lstream_count<item.groups_count)">&nbsp<a title="Необходимо сформировать учебные группы"><span style="color: red;"><i class="fa-solid fa-circle-exclamation"></i></span></span> 
                    </nobr>
                    <br />
            ...
            <td align="center"> 
                ...
                <div v-if="is_DEVELOPMENT &&  item.status_id>10 && item.status_id<16 " class="btn-group" class="col-1"   style="padding-bottom: 5px; padding-right: 0px;" >
                  <button  class="btn btn-light btn-sm text-success dropdown-toggle" data-bs-toggle="dropdown" aria-expanded="false"  title="Документы по образованию"><b><i class="fa-regular fa-file-lines"></i></b></button>
                    <ul class="dropdown-menu"  style="--bs-dropdown-min-width: 15rem; padding: 7px;">
                          ...
                          <li><a :href="'d-api/documents-cert.php?certificate=0&template=0&groupid=0&id='+item.order_id+'&print_v='+this.print_v" target="_blank"  class="nav-link"  title="Дипломы / Удостоверения" > &nbsp;<span class="icon"> <i class="fa-regular fa-file-lines"></i></span> Дипломы / Удостоверения</a></li>
                    </ul>
                </div>
...
<span v-if="role!='counterparty' && a_counterparty_id>0">
  ...
  <span v-if="a_order_id>0">
    <div class="mb-2">	
        <div align="right">
            <router-link :to="{ name: 'student_report'}" ><button  class="btn btn-outline-primary"> Закрыть </button></router-link>
        </div>
    </div>
  </span>
  <span v-else>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-body" align="left">
        ...
        <a v-if="modal_counterparty_id>1 && (this.role_privileges.orders_list_buh == 2 || this.role_privileges.orders_list_buh == 1)" :href="'d-api/documents-order.php?contract_id=100&id='+this.modal_order_id+'&print_v='+this.print_v" target="_blank"  class="nav-link"  title="Версия для печати"  > &nbsp;<span class="icon"> <i class="fa-regular fa-file-lines"></i></span> Счет </a>
        <div v-if="this.role_privileges.orders_list_buh == 2 || this.role_privileges.orders_list_buh == 1" class="row">
                <div class="col-md-4">
                  <a  :href="'d-api/documents-order.php?contract_id=101&id='+this.modal_order_id+'&enable_lock=1&print_v='+this.print_v" target="_blank"  class="nav-link"  title="Версия для печати"  > &nbsp;<span class="icon"> <i class="fa-regular fa-file-lines"></i></span> Акт выполненных работ </a>
                </div>
                <div class="col-md-2" style="padding-top: 5px; color: green;">
                    <span v-if="modal_completed.upload_file != null" ><i class="fa-solid fa-check"></i>&nbsp;&nbsp;&nbsp;&nbsp;<a :href="'d-api/documents-view.php?file='+modal_completed.upload_dir+modal_completed.upload_file"  target="_blank" title="Сохраненный скан"  style="padding-top: 0px; padding-bottom: 15px;"><button type="button" class="btn btn-secondary"><i class="fa-solid fa-file-pdf"></i></button></a></span>
                </div>
                <div class="col-md-5">
        ...
        <span v-for="(item2, index) in modal_contract_list">
            <p v-if="this.role_privileges.orders_list_buh == 2 || this.role_privileges.orders_list_buh == 1"    > 
                  <a :href="'d-api/documents-order.php?contract_id='+item2.contract_id+'&counterparty_id='+this.modal_counterparty_id+'&id='+this.modal_order_id+'&print_v='+this.print_v+'&paper=l'" class="text-decoration-none  link-body-emphasis"  target="_blank">&nbsp<i class="fa-solid fa-file-contract"></i>{{item2.name}}</a>&nbsp;
                  <span v-if="item2.longtime_contract_is_signed>0" >
                      <span style="color: green;">&nbsp<i class="fa-solid fa-check"></i></span>&nbsp;&nbsp;<a :href="'d-api/documents-view.php?file='+item2.upload_dir+item2.upload_file"  target="_blank" title="Сохраненный скан"  style="padding-top: 0px; padding-bottom: 15px;"><button type="button" class="btn btn-secondary"><i class="fa-solid fa-file-pdf"></i></button></a>
                  </span>
                  <span v-else  style="color: red;" title="Скан не загружен" ><i class="fa-solid fa-circle-exclamation"></i></span>
            </p>


            <!--<div v-if="item2.longtime_contract_is_signed==0 && this.role_privileges.orders_list_buh == 2 || this.role_privileges.orders_list_buh == 1"   class="row">-->
            <div v-if="this.role_privileges.orders_list_buh == 2 || this.role_privileges.orders_list_buh == 1"  class="row" > 
                <!--<div class="col-md-1" style="text-align: center; padding-top: 5px; color: green;">
                    <span v-if="item2.upload_file != '' &&  item2.longtime_contract==0"  ><i class="fa-solid fa-check"></i>&nbsp;&nbsp;&nbsp;&nbsp;<a :href="'d-api/documents-view.php?file='+item2.upload_dir+item2.upload_file"   target="_blank" title="Сохраненный скан"  style="padding-top: 0px; padding-bottom: 15px;"><button type="button" class="btn btn-secondary">&nbsp;&nbsp;<i class="fa-solid fa-print"></i>&nbsp;&nbsp;</button></a></span> 
                </div>-->
                <div class="col-md-1">
                </div>

                <div class="col-md-5">
                    <input type="file" v-model="contract_file[index*5]" placeholder="Скан договора"   class="form-control"  :id="'input_contract_file_'+index+'_'"  @change="handleUploadC(index, item2.upload_file)"    :ref="el => { if (el) divs[index*4] = el }"  >
                </div>
                <div class="col-md-1">
                    .pdf
                </div>
                ...
                <div class="col-md-1" style=" padding-top: 5px; color: red;"><span v-if="importfile1[index*5 +1 ]!=null || legacy_contract[index]!='' "><b>*</b></span></div>

                <div class="col-md-2" >
                      <input type="text" v-model="legacy_contract[index]"   placeholder="Номер"  title="Номер существующего договора (при наличии)"  class="form-control"  style="margin-right: 5px;"  @change="checkDate(index)"  >
                </div>
            </div>
            <div v-if="(this.role_privileges.orders_list_buh == 2 || this.role_privileges.orders_list_buh == 1) && item2.longtime_contract == 0  "  class="row" > 
                ...
                <div class="col-md-4">
                    {{new Date(this.modal_date_completed).toLocaleDateString("ru")}}
                </div>
            </div>
            ...
            <ul>
                <li v-if="item2.additions_count>=1 && this.role_privileges.orders_list_buh == 2 || this.role_privileges.orders_list_buh == 1" style=" list-style-type: none;"><div class="row">
                <div class="col-md-3" style="font-size: smaller;">
                    <li v-if="item2.additions_count>=1" ><a :href="'d-api/documents-order.php?contract_id='+item2.contract_id+'&addition=1&id='+this.modal_order_id+'&print_v='+this.print_v+'&paper=l'"  title="Версия для печати"  target="_blank"  class="nav-link">&nbsp<i class="fa-regular fa-file"></i>&nbspПриложение 1 </a></li>
                </div>
                <div class="col-md-2" style="text-align: right; padding-top: 5px; color: green;">
                    <span v-if="item2.additions_list[1].addition==1 && item2.additions_list[1].date_contract != '' && item2.additions_list[1].upload_file != ''" ><i class="fa-solid fa-check"></i>&nbsp;<a  :href="'d-api/documents-view.php?file='+item2.additions_list[1].upload_dir+item2.additions_list[1].upload_file"  target="_blank" title="Сохраненный скан"  style="padding-top: 0px; padding-bottom: 15px;"><button type="button" class="btn btn-secondary"><i class="fa-solid fa-file-pdf"></i></button></a></span>
                </div>
                <div class="col-md-6">
                    <input type="file" v-model="contract_file[index*5  +1 ]" placeholder="Скан договора"   class="form-control"  :id="'input_contract_file_'+index+'_1'"  @change="handleUpload(index, 1)"    :ref="el => { if (el) divs[index*4+1] = el }"  >
                </div>
                <div class="col-md-1">
                    .pdf
                </div>
                ...
                <li v-if="item2.additions_count>=2 && this.role_privileges.orders_list_buh == 2 || this.role_privileges.orders_list_buh == 1" style=" list-style-type: none;"><div class="row">
                <div class="col-md-3" style="font-size: smaller;">
                <li v-if="item2.additions_count>=2"><a :href="'d-api/documents-order.php?contract_id='+item2.contract_id+'&addition=2&id='+this.modal_order_id+'&print_v='+this.print_v+'&paper=l'"  title="Версия для печати"   target="_blank"  class="nav-link">&nbsp<i class="fa-regular fa-file"></i>&nbspПриложение 2 </a></li>
                </div>
                <div class="col-md-2" style="text-align: right; padding-top: 5px; color: green;">
                    <span v-if="item2.additions_list[2].addition==2 &&  item2.additions_list[2].date_contract != '' && item2.additions_list[2].upload_file != ''" ><i class="fa-solid fa-check"></i>&nbsp;<a :href="'d-api/documents-view.php?file='+item2.additions_list[2].upload_dir+item2.additions_list[2].upload_file"  target="_blank" title="Сохраненный скан"  style="padding-top: 0px; padding-bottom: 15px;"><button type="button" class="btn btn-secondary"><i class="fa-solid fa-file-pdf"></i></button></a></span>
                </div>
                <div class="col-md-6">
                    <input type="file" v-model="contract_file[index*5  +2 ]" placeholder="Скан договора"   class="form-control"  :id="'input_contract_file_'+index+'_2'"  @change="handleUpload(index, 2)"    :ref="el => { if (el) divs[index*4+2] = el }"  >
                </div>
                <div class="col-md-1">
                    .pdf
                </div>
                ...
                <div class="col-md-3" style="font-size: smaller;">
                 <li ><a :href="'d-api/documents-order.php?contract_id='+item2.contract_id+'&addition=0&id='+this.modal_order_id+'&print_v='+this.print_v+'&paper=l'"  title="Версия для печати"  target="_blank"  class="nav-link">&nbsp<i class="fa-regular fa-file"></i>&nbspЗаявление/Согласие на обработку персональных данных</a></li>
                </div>
                <div class="col-md-2" style="text-align: right; padding-top: 5px; color: green;">
                    <span  v-if="item2.additions_list[0].addition==0 &&  item2.additions_list[0].date_contract != '' && item2.additions_list[0].upload_file != ''"   ><nobr><i class="fa-solid fa-check"></i>&nbsp;<a :href="'d-api/documents-view.php?file='+item2.additions_list[0].upload_dir+item2.additions_list[0].upload_file"  target="_blank" title="Сохраненный скан"  style="padding-top: 0px; padding-bottom: 15px;"><button type="button" class="btn btn-secondary"><i class="fa-solid fa-file-pdf"></i></button></a></nobr></span>
                </div>
                <div class="col-md-6">
                    <input type="file" v-model="contract_file[index][index*5 ]" placeholder="Скан договора"   class="form-control"  :id="'input_contract_file_'+index+'_0'"  @change="handleUpload(index, 0)"   :ref="el => { if (el) divs[index*4+3] = el }" >
                </div>
                <div class="col-md-1">
                    .pdf
                </div>
      ...
      <div class="modal-footer">
        <span v-if="this.date_exist"> 
            <button v-if="date_exist"  class="btn btn-primary"    @click="contractSave()"  data-bs-dismiss="modal"  > Сохранить </button>
            <button v-else  class="btn btn-outline-secondary"    > Сохранить </button>
        </span>
        <span v-else> 
